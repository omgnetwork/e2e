...
Create a transaction request successfully
    # Build payload
    ${data}    Get Binary File    ${RESOURCE_PATH}/create_transaction_request.json
...
Get a transaction request successfully
    # Build payload
    ${data}    Get Binary File    ${RESOURCE_PATH}/get_transaction_request.json
...
Join transaction request channel successfully
    ...
    ${data}    Get Binary File    ${RESOURCE_PATH}/join_channel.json
...
Consume transaction request successfully as an admin
    ${data}    Get Binary File    ${RESOURCE_PATH}/consume_transaction_request.json
...
Approve transaction consumption successfully
    ${data}    Get Binary File    ${RESOURCE_PATH}/approve_transaction_consumption.json
    ${data}    Update Json    ${data}    id=${TRANSACTION_CONSUMPTION_ID}
    # Build authentication headers
    &{headers}    Build Authenticated Request Header
    # Perform request
    ${resp}    Post Request    api    ${CLIENT_TRANSACTION_CONSUMPTION_REJECT}    data=${data}    headers=${headers}
    # Assert response
    Assert Response Success    ${resp}
    Assert Object Type    ${resp}    transaction_consumption
